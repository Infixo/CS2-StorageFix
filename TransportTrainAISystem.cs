...
public class TransportTrainAISystem : GameSystemBase
{
    ...
    private struct TransportTrainTickJob : IJobChunk
    {
        ...
        private void Tick(int jobIndex, ref Random random, Entity vehicleEntity, Owner owner, PrefabRef prefabRef, CurrentRoute currentRoute, DynamicBuffer<LayoutElement> layout, DynamicBuffer<TrainNavigationLane> navigationLanes, DynamicBuffer<ServiceDispatch> serviceDispatches, bool isUnspawned, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref PathOwner pathOwner, ref Target target, ref Odometer odometer)
        {
            ...
            if (VehicleUtils.ResetUpdatedPath(ref pathOwner))
            {
                ...
                if (m_ConnectedData.HasComponent(target.m_Target) && m_BoardingVehicleData.HasComponent(m_ConnectedData[target.m_Target].m_Connected))
                {
                    float distance = num * 0.5f;
                    PathUtils.ExtendPath(path, pathOwner, ref distance, m_CurveData, m_LaneData, m_EdgeLaneData, m_OwnerData, m_EdgeData, m_ConnectedEdges, m_SubLanes);
                }
            ...
            else if (VehicleUtils.PathEndReached(currentLane))
            {
                if ((cargoTransport.m_State & (CargoTransportFlags.Returning | CargoTransportFlags.DummyTraffic)) != 0 || (publicTransport.m_State & (PublicTransportFlags.Returning | PublicTransportFlags.DummyTraffic)) != 0)
                {
                    if ((cargoTransport.m_State & CargoTransportFlags.Boarding) != 0 || (publicTransport.m_State & PublicTransportFlags.Boarding) != 0)
                    {
                        ...
                        {
                            flag = true;
                            if (!SelectNextDispatch(jobIndex, vehicleEntity, currentRoute, layout, navigationLanes, serviceDispatches, ref cargoTransport, ref publicTransport, ref currentLane, ref pathOwner, ref target))
                            {
                                VehicleUtils.DeleteVehicle(m_CommandBuffer, jobIndex, vehicleEntity, layout);
                                return;
                            }
            ...
            else if ((cargoTransport.m_State & CargoTransportFlags.Returning) != 0 || (publicTransport.m_State & PublicTransportFlags.Returning) != 0)
            {
                ...
                {
                    SelectNextDispatch(jobIndex, vehicleEntity, currentRoute, layout, navigationLanes, serviceDispatches, ref cargoTransport, ref publicTransport, ref currentLane, ref pathOwner, ref target);
                }
            }
        ...
        private void FindPathIfNeeded(Entity vehicleEntity, PrefabRef prefabRef, Entity skipWaypoint, ref TrainCurrentLane currentLane, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref PathOwner pathOwner, ref Target target)
        {
                ...
                if (m_PathElements.HasBuffer(request))
                {
                    ...
                    if (appendPath.Length != 0)
                    {
                        ...
                        if (PathUtils.TryAppendPath(ref currentLane, navigationLanes, dynamicBuffer, appendPath))
                        {
                            ...
                            cargoTransport.m_State &= ~CargoTransportFlags.Arriving;
                            publicTransport.m_State &= ~PublicTransportFlags.Arriving;
                            float num2 = VehicleUtils.CalculateLength(vehicleEntity, layout, m_PrefabRefData, m_PrefabTrainData);
                            PathElement prevElement = default(PathElement);
                            if (navigationLanes.Length != 0)
                            {
                                TrainNavigationLane trainNavigationLane = navigationLanes[navigationLanes.Length - 1];
                            ...
                            if (m_ConnectedData.HasComponent(target.m_Target) && m_BoardingVehicleData.HasComponent(m_ConnectedData[target.m_Target].m_Connected))
                            {
                                ...
                                PathUtils.ExtendPath(dynamicBuffer, pathOwner, ref distance, m_CurveData, m_LaneData, m_EdgeLaneData, m_OwnerData, m_EdgeData, m_ConnectedEdges, m_SubLanes);
                            }
                            return true;
                        }
                    }
        ...
        private bool StopBoarding(int jobIndex, ref Random random, Entity vehicleEntity, CurrentRoute currentRoute, DynamicBuffer<LayoutElement> layout, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref Target target, ref Odometer odometer, bool isCargoVehicle, bool forcedStop)
        {
            ...
            if (!forcedStop)
            {
                if (flag2 && (m_SimulationFrameIndex < cargoTransport.m_DepartureFrame || m_SimulationFrameIndex < publicTransport.m_DepartureFrame))
                {
                    return false;
                }
                if (layout.Length != 0)
    ...
    protected override void OnUpdate()
    {
        ...
        jobData.m_TransportVehicleRequestData = __TypeHandle.__Game_Simulation_TransportVehicleRequest_RO_ComponentLookup;
        ...
        jobData.m_PublicTransportVehicleData = __TypeHandle.__Game_Prefabs_PublicTransportVehicleData_RO_ComponentLookup;
        jobData.m_CargoTransportVehicleData = __TypeHandle.__Game_Prefabs_CargoTransportVehicleData_RO_ComponentLookup;
        ...
        JobHandle jobHandle3 = boardingData.ScheduleBoarding(this, m_CityStatisticsSystem, m_BoardingLookupData, m_SimulationSystem.frameIndex, jobHandle2);
